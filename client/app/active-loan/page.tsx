...
export default function ActiveLoanPage() {
    ...
    const fetchLoanData = async () => {
      try {
        ...
        if (loanData && loanData.active) {
          const principalAmount = Number(formatEther(loanData.principal));
          const interestAmount = Number(formatEther(loanData.interestAccrued));
          const totalAmount = principalAmount + interestAmount;
          const paymentPerInstallment =
            BigInt(Math.floor(totalAmount * 1e18)) / BigInt(3);
  ...
  const handleRepayment = async () => {
    try {
      ...
      const hash = await repayLoan(formatEther(loan.paymentAmount));
      toast({
            ...
            <CardContent className="space-y-4">
              <div>
                <div className="text-3xl font-bold">
                  cKES {Number(formatEther(loan.principal)).toLocaleString()}
                </div>
                <div className="text-sm text-muted-foreground">
                  Started on{" "}
                  {formatDate(
                    new Date(Number(loan.lastUpdate) * 1000).toISOString()
                  )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="font-medium">
                    cKES{" "}
                    {loan.paymentAmount
                      ? Number(formatEther(loan.paymentAmount)).toLocaleString()
                      : "0"}
                  </div>
                </div>
              </div>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-sm text-muted-foreground">
                              ...
                              {loan.paymentAmount
                                ? Number(
                                    formatEther(loan.paymentAmount)
                                  ).toLocaleString()
                                : "0"}
                            </div>
                          </div>
