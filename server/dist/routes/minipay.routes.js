...
const ethers_1 = require("ethers"); // npm install ethers
...
router.get("/challenge", (req, res) => {
    ...
    exports.nonceCache[address.toLowerCase()] = nonce;
    // Optionally expire the nonce after 5 minutes
    setTimeout(() => {
        delete exports.nonceCache[address.toLowerCase()];
    }, 5 * 60 * 1000);
...
router.post("/verify", (req, res) => {
    ...
    try {
        ...
        const cachedNonce = exports.nonceCache[address.toLowerCase()];
        if (!cachedNonce) {
        ...
        if (recovered.toLowerCase() !== address.toLowerCase()) {
            console.log("Signature mismatch. Recovered:", recovered, "Expected:", address);
            return res
                .status(400)
                .json({ error: "Signature does not match address" });
        }
        // Success! Optionally, create a session or JWT here.
        delete exports.nonceCache[address.toLowerCase()]; // Invalidate nonce after use
