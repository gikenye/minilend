...
exports.LendingPoolController = void 0;
const lending_pool_service_1 = require("../services/lending-pool.service");
class LendingPoolController {
    constructor() {
    ...
    async createLendingPool(req, res) {
        try {
    ...
    async getLendingPoolById(req, res) {
        try {
    ...
    async updateLendingPool(req, res) {
        try {
            const { poolId } = req.params;
            const updates = req.body;
            const pool = await this.lendingPoolService.updateLendingPool(poolId, updates);
            if (!pool) {
                res.status(404).json({ error: "Lending pool not found" });
                return;
            }
    ...
    async fundLendingPool(req, res) {
        try {
            ...
            const pool = await this.lendingPoolService.fundLendingPool(poolId, amount);
            if (!pool) {
                res.status(404).json({ error: "Lending pool not found" });
                return;
            }
    ...
    async withdrawFromLendingPool(req, res) {
        try {
            const { poolId } = req.params;
            const { amount } = req.body;
            if (!amount || amount <= 0) {
            ...
            const pool = await this.lendingPoolService.withdrawFromLendingPool(poolId, amount);
            if (!pool) {
                res.status(404).json({ error: "Lending pool not found" });
                return;
            }
    ...
    async contributeToPool(req, res) {
        try {
            ...
            const result = await this.lendingPoolService.contributeToPool(poolId, amount, miniPayAddress);
...
exports.LendingPoolController = LendingPoolController;
