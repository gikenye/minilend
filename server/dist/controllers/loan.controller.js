...
class LoanController {
    constructor() {
        this.calculateLoanLimit = async (req, res) => {
            try {
                const { address } = req.user;
                const creditScore = await this.creditScoreService.getCreditScore(address);
                const limitResult = await this.loanService.calculateLoanLimit(address, creditScore);
        ...
        this.loanHistoryService = new loan_history_service_1.LoanHistoryService();
        this.creditScoreService = new credit_score_service_1.CreditScoreService();
    }
    async getLoanLimit(req, res) {
        try {
            const miniPayAddress = req.headers["x-minipay-wallet"];
            if (!miniPayAddress) {
            ...
            const limitResult = await this.loanService.calculateLoanLimit(miniPayAddress, creditScore);
    ...
    async applyForLoan(req, res) {
        try {
            ...
            const loan = await this.loanService.processApplication(miniPayAddress, amountCUSD, amountLocal, localCurrency, termDays);
    ...
    async getLoanHistory(req, res) {
        try {
            ...
            const history = await this.loanHistoryService.getUserLoanHistory(miniPayAddress);
    ...
    async makeRepayment(req, res) {
        try {
            ...
            const result = await this.loanService.makeRepayment(loanId, amount, miniPayAddress);
